...
DATA=train
output_dir=../checkpoints/paper/finetuning_arabart/qalb15_${DATA}_synth
...
CUDA_VISIBLE_DEVICES=0 python -m gec.train \
  --train_file ../data/synthetic/mix_q15_arabil/train-2x19k_real-1x25k+arabil_synth_t10_excl_UC+1x25karabil_synth_t25.jsonl \
  ...
  --do_train --do_eval \
  --run_name PAPER_finetune_arabart_synth \
...
output_root=../outputs/paper/finetuning_arabart/qalb15_synth_${DATA}
output_path=$output_root/qalb15-test-l1-output.txt
model_path=${output_dir}_lr/checkpoint-17000
