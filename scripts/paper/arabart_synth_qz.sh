...
DATA=train
output_dir=../checkpoints/paper/finetuning_arabart/qalb15zaebuc_${DATA}_synth


CUDA_VISIBLE_DEVICES=0 python -m gec.train \
  --train_file ../data/synthetic/mix_q15_zaebuc_arabil/train-2x19k_real-1x25k+arabil_synth_t10_excl_UC+1x25karabil_synth_t25.jsonl \
  --validation_file ../data/real/clean/qalb-14+qalb-15+ZAEBUC/jsonl/dev.jsonl \
  --model_name_or_path moussaKam/AraBART \
  --output_dir ${output_dir}_lr${lr} \
  --do_train --do_eval \
  --run_name PAPER_finetune_arabart_z_synth \
...
output_root=../outputs/paper/finetuning_arabart/qalb15zaebuc_synth_${DATA}
output_path=$output_root/qalb15-test-l1-output.txt
model_path=${output_dir}_lr/checkpoint-11500
